...
Async job, runs after extraction on every episode close. Autonomous; never
hard-deletes — `MemoryStatus.MERGED` / `SUPERSEDED` retire a memory without
destroying it, and `memory_relations` records why (`merged_from`,
`supersedes`) for later provenance.

The Project Brain's quality guardian: extraction's write-time similarity
gate (§6) only catches near-exact duplicates. Reflection catches what it
misses — the same fact worded differently, later knowledge that contradicts
earlier knowledge, corroborating-but-distinct observations.

**Deterministic shortlist, LLM confirms** (never the other way round):
embedding similarity clusters active project memories against each other
(`update_similarity_threshold`); a cluster of one is nothing to review, and
most cycles produce zero clusters — no clusters means no model call. Each
cluster that does form is sent to the reasoner, which picks exactly one:

- **merge** — same statement, different words → one self-contained memory,
  the rest marked `MERGED`
- **supersede** — one is outdated by the other → the old one marked
  `SUPERSEDED`
- **strengthen** — distinct but corroborating → confidence bumped via the
  same reinforcement gate extraction uses (`MemoryRepository.touch()`)
- **keep** — genuinely distinct; the default when uncertain

Reflection only regenerates the Project State (§8) when it changed
...
memories (`project_states` table: project_id, version, content,
generated_from). Append-only — a new episode close that changes nothing
material produces no new version. `generated_from` records which memory ids
fed the synthesis (explainable, not reprocessed). Latest version is the
active overview; older versions stay queryable for the Dashboard's future
Timeline view.
